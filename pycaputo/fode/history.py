...
def make_history(state: Array, nsteps: int | None = None) -> History:
    """Make a history checkpointing instance for the given *state* type.

    :arg state: an array of the same shape and :class:`~numpy.dtype` as the
        results generated by the time stepper.
    :arg nsteps: number of time steps that will be taken, if known in advance.
    """
    if nsteps is None:
        return VariableProductIntegrationHistory()
    else:
        return FixedSizeHistory.empty(nsteps, state.shape, dtype=state.dtype)
...
class FixedSizeHistory(History):
    ...
    def empty(
        ...
        *,
        ...
        """Construct a :class:`FixedSizeHistory` for given sizes.

        :arg n: number of time steps that will be stored.
        :arg d: dimension of each state array.
        """
        return cls(
            history=np.empty((n, *shape), dtype=dtype),
